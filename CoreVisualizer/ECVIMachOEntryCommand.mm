...
{
	if ((self = [super initWithBinary:binary type:cmd->cmd size:cmd->cmdsize cmd:cmd])) {
		...
		if (cmd->cmd == LC_THREAD || cmd->cmd == LC_UNIXTHREAD) {
			struct arch_thread_command *realcmd = reinterpret_cast<struct arch_thread_command *>(cmd);
			...
			struct entry_point_command *realcmd = reinterpret_cast<struct entry_point_command *>(cmd);
